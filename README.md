...
3. Normalizes equation syntax (lowercase + canonical functions)  
4. Generates NumPy-ready equation strings  
5. Parses applicability ranges  
6. Builds a relational SQLite database  
...
- Direct interoperability with Python / NumPy
...
- Proper joining of:
  ...
  - `fuente_referencia`
- UTF-8 safe output (via `readr::write_csv()`)

---

## A3 – Equation Normalization (Lowercase + Canonical Form)

Each equation is stored in three versions:

- `ecuacion_raw` (verbatim from source)
- `ecuacion_normalizada` (canonical lowercase form)
- `ecuacion_numpy` (NumPy-ready expression)

### Lowercase Standardization

All equations are converted to lowercase to eliminate case inconsistencies:
...
is generated to allow direct evaluation in Python using NumPy.

### Function Mappings

| Original Form             | NumPy Equivalent        |
|--------------------------|------------------------|
| `exp(x)`                 | `np.exp(x)`            |
| `ln(x)`                  | `np.log(x)`            |
| `potencia(x,y)`          | `np.power(x,y)`        |
| `pow(x,y)`               | `np.power(x,y)`        |
| `raiz(x)` or `sqrt(x)`   | `np.sqrt(x)`           |
| `abs(x)`                 | `np.abs(x)`            |
...
Exp(-10.01137401 + 1.97688779Ln(Diam) + 1.02860759Ln(Alt))


NumPy-ready:

np.exp(-10.01137401 + 1.97688779np.log(diam) + 1.02860759np.log(alt))
...
0.00006*Potencia(diam,1.98226)*Potencia(alt,0.84224)


NumPy-ready:

0.00006*np.power(diam,1.98226)*np.power(alt,0.84224)


This allows later evaluation in Python:
